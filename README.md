...
- **数据库**: SQLite (better-sqlite3 12.5)
...
# 文件目录（可选，下方为示例值，实际默认使用 /tmp/chuangcut）
# RUNTIME_DIR=/tmp/chuangcut       # 运行时根目录
# TEMP_DIR=/tmp/chuangcut/temp     # 临时文件
# OUTPUT_DIR=/tmp/chuangcut/output # 输出文件
...
├── scripts/               # 运维脚本
│   ├── dev.sh            # 开发环境启动
│   ├── test.sh           # 测试入口（unit/integration/e2e/coverage）
│   ├── build-multiplatform.sh  # 多平台 Docker 构建
│   ├── publish-docker.sh # Docker 镜像发布
│   └── entrypoint.sh     # 容器入口脚本
├── docs/agent/            # 技术文档（架构、API、工作流等）
...
- 个人用户、快速上手 → 选择 AI Studio（限制 300MB）
