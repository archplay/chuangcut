...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // 清除对应的 Gemini 客户端缓存，确保使用新凭证
    if (data.service === 'google_ai_studio') {
      geminiClient.clearCache('ai-studio')
    } else if (data.service === 'google_vertex') {
      geminiClient.clearCache('vertex')
    }
