...
const createJobSchema = z.object({
  input_videos: z.array(videoInputSchema).min(1, '至少需要 1 个视频').max(5, '最多支持 5 个视频'),
  style_id: z.string().min(1, 'style_id 不能为空'),
  config: z
    .object({
      max_concurrent_scenes: z.number().int().min(1).max(5).optional(),
      storyboard_count: z.number().int().min(3).max(30).optional(),
