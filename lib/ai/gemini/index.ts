...
export class GeminiClient implements IGeminiClient {
  ...
  /**
   * 清除客户端缓存
   * 当 API Key 更新后调用，确保下次使用新凭证创建客户端
   */
  clearCache(platform?: GeminiPlatform): void {
    if (platform) {
      this.clientCache.delete(platform)
      logger.info(`Gemini 客户端缓存已清除: ${platform}`)
    } else {
      this.clientCache.clear()
      logger.info('Gemini 客户端缓存已全部清除')
    }
  }
